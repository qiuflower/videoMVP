...
## 概述 (Overview)
本技能读取改写后的 `storyboard.csv`，并根据结构模板将景别、运动镜头、新焦段、镜头方位、新画面内容、台词、画面风格及参考人像角色融合，生成包含精细画面控制要素的视频重绘提示词，并追加导出。

## 适用场景 (When to Use)
在剧本改写完成后，为方便将提示词批量拷贝导入 AI 视频重绘引擎（如 Runway, Kling, Sora）时使用。
...
2. 提示词内容应符合包含焦段、镜头方位及画面风格的精细重绘标准格式模板。
